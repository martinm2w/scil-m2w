...
//<<<<<<< SentenceSplitter.java
    public ArrayList splitChinese(String content) {
        ...
        for(int i=0;i<totalLength;i++){
            ...
            /* TODO Check new word
            //Check if there's a space before a char
            if(wordCount==0 && c==SPACE){
                //It's a new sentence
                
            }*/
            
            //Check if a new line is a new sentence
            if(c==NEWLINE){
                boolean isNewSentence=false;
                boolean stopSearch=false;
                //Iterator for checking the new line
                for(int j=i+1;j<totalLength && isNewSentence==false && stopSearch==false;j++ ){
                    char next=content.charAt(j);
                    if(Character.isLetter(next)){
//                         not the end of last sequence i.e. 
//                        掩飾罪
//                     行。                    
                    
                        //isNewSentence=false;
                        stopSearch=true; //stop search because it's not a new setence
                    } else{
                        //new sentence
                        isNewSentence=true;
                    }
//                    
//                    /* Check new line begin as list or just digit char
//                     * list: 
//                     * 那年我
//                     * 1. what
//                     * 2. blabla
...
//                     */
                    if(Character.isDigit(next)){
                        //Assuming as a list
                        for(int k=j+1;k<totalLength;k++){
                            char charNextToDigit=content.charAt(k);
                            // Check puncutation next to digit
                            if(charNextToDigit!=PERIOD && 
                                    charNextToDigit!=cnPeriod &&
                                    charNextToDigit!=SPACE && 
                                    charNextToDigit!=cnColon &&
                                    charNextToDigit!=')' &&
                                    !Character.isDigit(charNextToDigit)){
                                stopSearch=true;
                            } else if (charNextToDigit==')'|| charNextToDigit==PERIOD){
//                                /*i.eA那年我
//                                *     7.5岁 
//                                *     那年我
//                                *     7):岁
//                                *   In case A, it's not a new sentence
//                                * B 那年我
//                                *   7. 岁
//                                *   那年我
//                                *   7) 岁
...
//                
//                //Split the utterence to new sentence
                if(isNewSentence){
                String s=new String();
                s=content.substring(sentenceStartIndex,i+1);
                s=s.trim();
                sentences.add(s);
                //set next sentence as start index
                sentenceStartIndex=i+1;
                
                }              
                
            }
//            // Does . or 。 mean a new sentence?
            if(c==PERIOD||c==cnPeriod){
                boolean isNewSentece=false;
                boolean stopSearch=false;
    		...
    		if ( i - 1 < 0 )
                ...
                // if there is something other than a space or newline or number after this then its not an end of sentence.
//                i.e. "i.e."  那我。。7.5岁
                if(!isNewSentece && !stopSearch&& content.charAt(i+1)!=NEWLINE
                        && content.charAt(i+1)!=SPACE && content.charAt(i+1)!=cnPeriod
                        && !Character.isDigit(content.charAt(i+1))){
                    stopSearch=true;
                    isNewSentece=true;
			    ...
			    {
    				// check if we are looking at a decimal number i.e. 2.0
    				if ( !isNewSentece && !stopSearch &&
                                        Character.isDigit(content.charAt(i+1)) //.0
                                        && Character.isDigit(content.charAt(i-1))){ //2.
                                    stopSearch=true;
                                }
			    }
                ...
                if(!isNewSentece && !stopSearch 
                        && i-1>0
                        && Character.isLetter(content.charAt(i-1)) ) {
                
                    //Check Abbrevation
                    if(Character.isUpperCase(content.charAt(i-1))){
                        stopSearch=true;
                    }                   
                }   else if(!isNewSentece && !stopSearch){
                    isNewSentece=true;
                }
//                    
//                // look for 。。。 or 。。。。
                    // 。+。 i.e. “美国《时代》周刊10日发表文章........”本人订阅了Time
//                  while(content.charAt(i+1)==cnPeriod || content.charAt(i+1)==PERIOD){
//                        isNewSentece=true;
//                        // adjust variables to jump to the end of this and end the sentence
//                        i++;
//                        }
//                    }
//                    /**
//                     * Check specified number of ..
//                     * if(content.charAt(i+1)==cnPeriod || content.charAt(i+1)==PERIOD){
//                        }
//                     */
    		if ( !stopSearch && i + 2 < totalLength )
			    {   
                               ...
                               // 。+。。 i.e. “美国《时代》周刊10日发表文章。。。”本人订阅了Time
    				if ( ( content.charAt( i + 1 ) == cnPeriod && content.charAt( i + 2 ) == cnPeriod )||
                                        (content.charAt( i + 1 ) == PERIOD && content.charAt( i + 2 ) == PERIOD ))
				    {
    					// adjust variables to jump to the end of this and end the sentence
    					isNewSentece = true;
    					i = i + 2;  //..+any
                                ...
                                //。+ 。。。
    				else if ( i+3 < totalLength && ((content.charAt( i + 2 ) == cnPeriod && content.charAt( i + 3 ) == cnPeriod)||
                                        content.charAt( i + 2 ) == PERIOD && content.charAt( i + 3 ) == PERIOD))
				    {
    					isNewSentece = true;
    					i = i + 3;
				    }
