...
//                    System.out.println("TurnNo:"+utterance.getTurn()+":\tREQD::: "+required);
                        if ((j > 0 && words[j - 1].trim().equalsIgnoreCase("So")) && (tagcontent.toLowerCase().startsWith("so"))) {
                            required = Ngram.filterUtterance(required);
                            feature.add(required);
                        } else if (j > 0 && words[j - 1].trim().equalsIgnoreCase("please")) {
...
//                        System.out.println("TurnNo:"+utterance.getTurn()+":\tplease+Verb==="+required);
                            ...
                            words[j - 2] = words[j - 2].replaceAll("/+([A-Z]+)*", " ");
                            if (words[j - 2].trim().equalsIgnoreCase("we") && words[j - 1].trim().equalsIgnoreCase("should")) {
//                        System.out.println("TurnNo:"+utterance.getTurn()+":\twe+should==="+required);
                                required = Ngram.filterUtterance((words[j - 2] + words[j - 1]));
                                feature.add(required);
                            }
                            /*
                             * else if(j>1 &&
                             * words[j-2].trim().equalsIgnoreCase("I") &&
                             * (words[j-1].trim().equalsIgnoreCase("will") )){
                             * required=Ngram.filterUtterance((words[j-2]+words[j-1]));
                             * feature.add(required); //
                             * System.out.println("TurnNo:"+utterance.getTurn()+":\tI
                             * will "+required); } else if(j>1 &&
                             * words[j-2].trim().equalsIgnoreCase("I") &&
                             * (words[j-1].trim().equalsIgnoreCase("would") )){
                             * required=Ngram.filterUtterance((words[j-2]+words[j-1]));
                             * feature.add(required); //
                             * System.out.println("TurnNo:"+utterance.getTurn()+":\tI
                             * would "+required);
                ...
                if (j > 0 && words.length > 0) {
                    ...
                    words[j] = words[j].replaceAll("/+([A-Z]+)*", " ");
                    required = (words[j - 1] + words[j]);
//                System.out.println(utterance.getTurn()+":"+required);

                    if (j > 0 && words[j - 1].trim().equalsIgnoreCase("I") && (words[j].trim().equalsIgnoreCase("suggest"))) {
                        required = Ngram.filterUtterance(required);
                        feature.add(required);
//                    System.out.println("TurnNo:"+utterance.getTurn()+":\tI suggest "+required);
                    } else if (j > 0 && words[j - 1].trim().equalsIgnoreCase("I") && (words[j].trim().equalsIgnoreCase("recommend"))) {
                        required = Ngram.filterUtterance(required);
                        feature.add(required);
//                    System.out.println("TurnNo:"+utterance.getTurn()+":\tI recommend "+required);
                    } else if (j > 0 && words[j - 1].trim().equalsIgnoreCase("ought") && (words[j].trim().equalsIgnoreCase("to"))) {
                        required = Ngram.filterUtterance(required);
                        feature.add(required);
//                    System.out.println("TurnNo:"+utterance.getTurn()+":\tought to "+required);
                    } else if (j > 2) {
                        words[j - 3] = words[j - 3].replaceAll("/+([A-Z]+)*", " ");
                        words[j - 2] = words[j - 2].replaceAll("/+([A-Z]+)*", " ");
                        if (words[j - 3].trim().equalsIgnoreCase("i") && words[j - 2].trim().equalsIgnoreCase("am") && (words[j - 1].trim().equalsIgnoreCase("going")) && (words[j].trim().equalsIgnoreCase("to"))) {
                            required = Ngram.filterUtterance((words[j - 3] + words[j - 2] + words[j - 1]) + words[j]);
                            feature.add(required);
//                    System.out.println("TurnNo:"+utterance.getTurn()+":\tought to "+required);
                        }
                    } else {
...
//     System.out.println(utterance.getTurn()+":::AFter WikiAD==="+content);            
...
//                term = ngrams.get(j);
                 ...
                 *
                 * for(int j = 0; j < ngrams.size(); j++){ String term =
                 * ngrams.get(j); utnumber=utterance.getTurn();
                 *
                 * if ((Settings.getValue(Settings.LANGUAGE)).equals("english")
                 * ){ if( (dr_ngrams_.containsKey(term)) |
                 * term.equals("disagree-reject")){ //
                 * System.out.println(utterance.getTurn()+"ngrams:"+term);
                 * double[] dr_ngram = dr_ngrams_.get(term); if(dr_ngram!=null
                 ...
                 * System.out.println(utterance.getTurn()+":term:"+term+":In
                 * LoopDR:"+dr_ngram[0]); // tmp.add(ngrams);
                 * if(tno.contains(utnumber)){ adrterms.add(term);
                 * drAndad.add(dr_ngram[0]); ad_dr.put(term,dr_ngram[0]);
                 *
                 * }
                 * else if(!tno.contains(utnumber)){ tno.add(utnumber);
                 *
                 * adrterms.clear(); adrterms.add(term);
                 *
                 * ad_dr.clear(); ad_dr.put(term,dr_ngram[0]);
                 *
                 * drAndad.clear(); drAndad.add(dr_ngram[0]); }
                 *
                 * }
                 *
                 * }
                 * else if( ad_ngrams_.containsKey(term)
                 * |term.equals("action-directive")){ //
                 * System.out.println(utterance.getTurn()+"ngrams"+term);
                 * double[] ad_ngram = ad_ngrams_.get(term); if(ad_ngram!=null
                 * ){ int one=term_frequency_.get(term); //
                 * System.out.println("tf==="+one); adrterms.add(term); //
                 * System.out.println(utnumber); //
                 * System.out.println(utterance.getTurn()+":term:"+term+":In
                 * LoopAD:"+ad_ngram[0]); if(tno.contains(utnumber)){
                 * adrterms.add(term); ad_dr.put(term,ad_ngram[0]);
                 * drAndad.add(ad_ngram[0]); } else if(!tno.contains(utnumber)){
                 * tno.add(utnumber);
                 *
                 * adrterms.clear(); adrterms.add(term);
                 *
                 * ad_dr.clear(); ad_dr.put(term,ad_ngram[0]);
                 *
                 * drAndad.clear(); drAndad.add(ad_ngram[0]);
                 *
                 * }
                 * }
                 * }
                 ...
                 *
                 * }
                 * //check if it has disagree-reject/action-directive/any other
                 * DR term boolean dr=false; Comparator
                 * c=Collections.reverseOrder(); Collections.sort(drAndad,c);
                 * for(Double s:drAndad){ System.out.println("drAndad="+s); }
                 * if(!ad_dr.isEmpty()){ for(String s:ad_dr.keySet()){ double
                 * tmpadr=0.0; tmpadr=ad_dr.get(s);
                 *
                 * if(drAndad.get(0)==tmpadr){
                 * System.out.println(utnumber+"\tHighest
                 * valueandterm=::"+s+tmpadr+"\n"); String tmp=s;                  *
                 * // if(tmp.trim().equals("disagree-reject")){ ////
                 * ngrams.clear(); //// ngrams.add(DsarmdDATag.DR); // dr=true;
                 * // break; // } // else if(
                 * tmp.trim().equals("action-directive")){//dr!=true &&
                 * str=DsarmdDATag.AD; ngrams.clear();
                 * ngrams.add(DsarmdDATag.AD); break; }
                 *
                 * else{ break; // str=DsarmdDATag.DR; } } } }
                 */
                for (int j = 0; j < ngrams.size(); j++) {
