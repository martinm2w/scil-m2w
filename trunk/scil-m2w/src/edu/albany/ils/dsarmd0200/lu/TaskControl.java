...
/**
 * @Author: Ting Liu @Date: 01/08/2010 This class is used to judge which
 * particant in a small discussion group has the most power defining the goal
...
public class TaskControl {
     ...
     * *******************************set attributes*********************
     */
    public void setUtterances(ArrayList utts) {
        utts_ = utts;
    }

    public void calTotalIrAdPmOc() {
        total_ac_oc_ = 0;
    ...
    public void processAD(NounList nls_) {
        if (Assertions.ddt_.getType().equals(DocDialogueType.ME)) {
            return;
        }
        for (int i = 0; i < utts_.size(); i++) {
            Utterance utt_ = (Utterance) utts_.get(i);
            ...
            if (uttc_nt_ == null) {
                uttc_nt_ = utt_.getContent();
            }
            if (uttc_ == null) {
                uttc_ = utt_.getTaggedContent();
            }
            if (uttc_ == null) {
                continue;
            }
            ...
            if (ws.length < 15) {
                ...
                for (int k = 0; k < ws.length; k++) {
                    /*
                     * if (ws.length > 1) { System.out.println("last two words:
                     * " + ws[ws.length -2] + " " + ws[ws.length -1]); }else {
                     * System.out.println("last one words: " + ws[ws.length
                     * -1]); }
                     */
                    String ws1 = ws[k].toLowerCase();
                    String[] ws1s = ws1.split("[\\W]+");
                    /*
                     * System.out.println("ws1s length: " + ws1s.length); for
                     * (int j = 0; j < ws1s.length; j++) {
                    ...
                    if (ws1s.length == 2) {
                        ...
                        if (ws1s[1].startsWith("n")
                                && (nls_.isPerson(ws1s[0])
                                || (ws1s[0].length() > 0 && nls_.isPerson(ws1s[0].substring(0, ws1s[0].length() - 1))))
                    ...
                    /*
                     * String ws1 = ws[ws.length - 1].toLowerCase(); String[]
                     * ws1s = ws1.split("[\\W]+"); System.out.println("ws1s
                     * length: " + ws1s.length); for (int j = 0; j <
                     * ws1s.length; j++) { System.out.println("ws1s[" + j + "]:
                     * " + ws1s[j]); } if (ws1s.length == 2) { if
                     * (ws1s[1].startsWith("n") && (nls_.isSpeaker(ws1s[0]) ||
                     * nls_.isPerson(ws1s[0]))) { if (!isLocalTopic(ws1s[0])) {
                     * System.out.println("find a person name: " + uttc_ + "
                     * ----- speaker: " + utt_.getSpeaker());
                     * utt_.setTag("action-directive"); } } else if (ws.length >
                     * 1) { ws1 = ws[ws.length - 2].toLowerCase(); ws1s =
                     * ws1.split("[\\W]+"); if (ws1s.length > 1 &&
                     * ws1s[1].startsWith("n") && (nls_.isSpeaker(ws1s[0]) ||
                     * nls_.isPerson(ws1s[0]))) { if (!isLocalTopic(ws1s[0])) {
                     * System.out.println("find a person name: " + uttc_ + "
                     * ----- speaker: " + utt_.getSpeaker());
                     * utt_.setTag("action-directive"); } } } } else
                     * System.out.println("no pos tag, last one word: " +
                     * ws[ws.length - 1]);
                     */
                }
                //}
    ...
    public boolean isNewSpk(Utterance utt_pos, int i) {
        ...
        if (uttc_ == null) {/*
             * System.out.println("post utterance is empty!!!!!");
             */ return false;
        }
        String[] ws = uttc_.split("[\\W]+");
        //String[] ws = uttc_.split("[\\s]+");
    ...
    public void calTkC(PtsReportXMLParser prxmlp, ArrayList spks, NounList nls_) {
        ...
        /*
         * calTotalPM(); System.out.println("total_ac_oc_: " + total_ac_oc_);
         * System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
         * System.out.println("++++++++++++++++++++++++++++++++\ncalculate Task
         * Control - PMI quintile"); calPMI(prxmlp, spks); calTotalPMS();
         * System.out.println("total_ac_oc_: " + total_ac_oc_);
         * System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
         * System.out.println("++++++++++++++++++++++++++++++++\ncalculate Task
         * Control - PMSI quintile"); calPMSI(null, spks); calTotalDTSI();
         * System.out.println("total_ac_oc_: " + total_ac_oc_);
         * System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
         * System.out.println("++++++++++++++++++++++++++++++++\ncalculate Task
         * Control - DTSI quintile"); calDTSI(null, spks);
         */
    }

    public void calIrDIPm(PtsReportXMLParser prxmlp, ArrayList ordered_spks) {
        Iterator keys = spks_.keySet().iterator();
        while (keys.hasNext()) {
            String key = (String) keys.next();
            Speaker spk = spks_.get(key);
            spk.calIrDIPm(utts_, total_ac_oc_);
        }
        ...
        while (keys.hasNext()) {
            ...
            if (spk1.getDICount() >= threshold_) {
                over_thr = true;
            }
            if (spk1.getDICount() >= str_threshold_) {
                over_str_thr = true;
            }
            if (Assertions.ddt_.getType().equals(Assertions.ddt_.QA)) {
                if (spk1.getDICount() >= threshold_QA_) {
                    over_thr = true;
                }
                if (spk1.getDICount() >= str_threshold_QA_) {
                    over_str_thr = true;
                }
            }
        }
    ...
    public boolean calDI(PtsReportXMLParser prxmlp, ArrayList ordered_spks) {
        Iterator keys = spks_.keySet().iterator();
        while (keys.hasNext()) {
            String key = (String) keys.next();
            Speaker spk = spks_.get(key);
...
//        System.out.println("only Action Directive is not enough!!!"); //commented by m2w 3/22/12 11:09 AM
    ...
    public void calDTSI(PtsReportXMLParser prxmlp, ArrayList ordered_spks) {
        ...
        while (keys.hasNext()) {
            ...
            if (spk1.getDTSICount() >= threshold_) {
                over_thr = true;
            }
            if (spk1.getDTSICount() >= str_threshold_) {
                over_str_thr = true;
            }
        }
    ...
    public void calPMSI(PtsReportXMLParser prxmlp, ArrayList ordered_spks) {
        ...
        while (keys.hasNext()) {
            ...
            if (spk1.getPMSICount() >= threshold_) {
                over_thr = true;
            }
            if (spk1.getPMSICount() >= str_threshold_) {
                over_str_thr = true;
            }
        }
        evaluate(dis, null, spks, over_thr, over_str_thr, prxmlp, ordered_spks);
    ...
    public void calPMI(PtsReportXMLParser prxmlp, ArrayList ordered_spks) {
        ...
        while (keys.hasNext()) {
            ...
            if (spk1.getPMICount() >= threshold_) {
                over_thr = true;
            }
            if (spk1.getPMICount() >= str_threshold_) {
                over_str_thr = true;
            }
        }
        evaluate(dis, null, spks, over_thr, over_str_thr, prxmlp, ordered_spks);
    ...
    public void evaluate(ArrayList dis,
        ...
        /*
         * double[] dis1 = new double[7]; dis1[0] = 0; dis1[1] = 0; dis1[2] = 0;
         * dis1[3] = 10; dis1[4] = 23; dis1[5] = 29; dis1[6] = 39; double[]
         * qt_thrs = Util.genQTThrs(dis1);
         */
    ...
    public boolean hasResponse(Utterance utt_, ArrayList utts_) {
        ...
        for (int i = 0; i < utts_.size(); i++) {
            ...
            if (utt.getRespTo() != null &&
                    utt.getRespTo().equals(turn_no)
